...
std::span<std::byte> MappedRegion::bytes() const noexcept {
  return valid() ? std::span(static_cast<std::byte *>(address_), size_)
                 : std::span<std::byte>();
}
...
PthreadOwner::~PthreadOwner() noexcept {
  if (joinable()) pthread_detach(thread_);
}

PthreadOwner::PthreadOwner(PthreadOwner &&other) noexcept
    : thread_(other.thread_), started_(other.started_), joined_(other.joined_) {
...
int PthreadOwner::start(void *(*entry)(void *), void *argument) noexcept {
...
pid_t ChildProcess::release() noexcept { return std::exchange(pid_, -1); }
