...
static void write_root_script(void) {
  ...
  int n = snprintf(
      ...
      "fi\n"
      "KMI=\"${AVER}-${KVER}\"\n"
      "# Safe mode: with temp root + permissive SELinux already in hand (and\n"
      "# before the module manager mounts anything), drop a 'disable' marker into\n"
      "# every module so a boot-looping module is skipped on this and later boots.\n"
      "# Same marker KernelSU/APatch use for `module disable`; reversible.\n"
