...
static void write_root_script(void) {
  ...
  int n = snprintf(
      ...
      "fi\n"
      "# on android 15 oplus/coloros, load_policy invalidates every running\n"
      "# process SID. init detects its own stale SID and calls abort(),\n"
      "# which kills pid 1 and triggers an instant kernel panic / reboot.\n"
      "# skip the reload entirely: kernelsu patches the in-kernel policy\n"
      "# struct directly after late-load, without a reload event.\n"
      "KSU_ALREADY=0\n"
      ...
      "fi\n"
      "# re-enable enforcing; kernelsu handles policy internally\n"
