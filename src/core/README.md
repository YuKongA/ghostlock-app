...
- `route/`: route selection, status, per-route contexts and their route-level
  ...
  `select_stack_route.cpp`), the pluggable `route_policy.hpp` registry
  (middleware policies with their static route hooks) and the
  pipeline/orchestrator (`pipeline.hpp` / `orchestrator.hpp`).
