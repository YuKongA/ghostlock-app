...
- Migration status (CPP00–CPP14): C++20 with static libc++; `PiRace`,
  `TcpZerocopyRoute`, `SelectStackRoute` and the Heap-side owners (`MmContextSet`, `UniqueFd`, `unique_ptr` skb buffer,
  payload pages) are
...
- Top-level files: executable orchestration, PI race, profile transport, target ABI and low-level shared helpers. They
  remain top-level because they connect several categories.
