 ...
 * cap) and decodes it into the transport struct. */
    int read_glk1_stdin(struct ghostlock::profile::kernel_offsets *out, char *release_buf,
                        size_t release_buf_cap);

    /* Reads a GLK1 v2 document from a file path. */
    int read_glk1_file(const char *path, struct ghostlock::profile::kernel_offsets *out,
