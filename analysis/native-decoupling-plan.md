...
- [ ] 用户确认后勾选真机门禁和阶段总项；TODO 只有在代码注释与登记项同时删除后才算完成。
- [ ] 每阶段实现和真机测试通过后，更新 `analysis/routes.md` 的“核心维护图：三路线端到端主链”，只反映该阶段已经验证的结构变化。
...
- [x] 文档按语言拆为英文 `.md` 与中文 `_ZH.md`，同语种链接闭合；四个版本文件各自包含完整字段和分层 execution 说明，不再依赖共有模板文档；支持设备表迁至 profile 目录。
