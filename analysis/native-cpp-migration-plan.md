...
- [ ] `VictimProcess` 完整收编（`child_pipes` 的 3 对 pipe + child 退休）：`parked_cmd_w` 的裸值语义需要精细等价证明，登记为后续批次。
