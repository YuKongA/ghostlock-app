...
| KERNEL-PANIC-01 | Multicast 攻击存在间歇性内核崩溃（无 dmesg/pstore 权限）：CPP06c、CPP07、CPP12b/c/e、CPP12k 分别在 spray 阶段或 W2 后触发；与构建布局可能相关，但 `CPP14-release` 证明**同一二进制**也可能一次中断、一次通过（Release 首次 W1b spray 后重启、复跑 8/8 route 通过），说明存在独立于构建的间歇性成分 | 无（需外部 panic 解析能力或多次统计）；布局敏感改动建议多次冷机复测 | 攻击层观察 | [ ] 已记录全部证据（`analysis/device-gates/CPP*-kernel-panic*`、`pi-timeout-binary-diff.md`、`CPP12k`/`CPP12l`、`CPP14-release` crash/pass）；不阻塞迁移推进 |
