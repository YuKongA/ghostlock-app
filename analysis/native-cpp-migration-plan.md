...
- [x] 回补 `SESSION-04`：resident stop 的 Heap 释放已移入 `ExploitSession::release_resident_heap()`（`kernel5_resident_stop` 只做 disarm/destroy，W1/W2 stop 顺序不变）；native 待冷机门禁确认（2026-09-18）。
- [ ] 回补 `SESSION-02`：Heap handoff 与 victim/handoff 收归随 session 编排批次。
...
- [x] Heap owner 段（`CPP07-OWNER` 主体）：删除 `common.h` 的 `page_base`/`last_mm_struct`/`fake_*`/`memfd_leak` 别名，调用点显式访问 `g_heap_context`（SYSCHK 字符串化经 `SYSCHK_pr` 保持，重建 native 与 `7b60739a…` 逐字节一致，`4ba123a`）；`mm_ctx` 的 calloc/free 数组 → `ghostlock::MmContextSet`（owning vector，析构只释放内存）、`leak_memfd` → `UniqueFd`、`skb_buffer` → `unique_ptr`，`9a18262`。攻击关键函数指令形状一致；Multicast 门禁通过 `CPP12h-20260917-multicast-pass`（native `328a6415…`）。剩余：prepare 失败注入框架；`leak_child` 已收编为 `ghostlock::ChildProcess` + `mark_reaped()`（显式表达 spray 路径的 `waitpid` 回收，scope 退出不再二次 signal）。
...
- [ ] VictimContext pid 所有权（`set_child`/`mark_child_exited`/`release_child`）：首次尝试 `c222151`（native `b147df9f`）在 APK 242 上 **2/2 kernel panic**（`CPP12k-20260917-multicast-kernel-panic`，均截断于 W2 spray 窗口），已回退（`552c8b2`），隔离复跑 PASS（`CPP12l`）。第二次等效重试（header-only pid 成员、无新 TU）经构建对比产生与 `c222151` **逐字节相同**的 `b147df9f`（8/8 攻击关键函数严格一致、仅 `run_exploit` +5 指令与 268 处地址注解位移），故按“同一构建 2/2 panic”先例**不再重复门禁**并回退。保留已验证的 pipe 集合版本（`625d5300…`）；pid 所有权待 `KERNEL-PANIC-01` 有进一步手段后再评估。
...
| KERNEL-PANIC-01 | Multicast 攻击存在间歇性系统崩溃：CPP06c、CPP07、CPP12b/c/e、CPP12k 分别在 spray 阶段或 W2 后触发；`CPP14-release` 与 `CPP12-victim-pid-clean-2run` 均证明**同一二进制**可一次崩溃、一次通过，且崩溃集中在 **W2 KernelSnitch collision / mm spray 窗口**（另有 W1b spray 后一例）。`CPP12-victim-pid-kernelsu-crash` 的 dmesg 捕获（root）显示一类**环境相关分支**：KernelSU 已加载时攻击触发 re-enforce → system_server 崩溃（无内核 panic 打印） | 攻击层时序/状态窗口；布局敏感改动仍需多次冷机复测；门禁必须在 KernelSU 未加载的干净启动下进行 | 攻击层观察 | [ ] 已记录全部证据（`analysis/device-gates/CPP*-kernel-panic*` 含 dmesg 捕获、`pi-timeout-binary-diff.md`、`CPP12k`/`CPP12l`、`CPP14-release`、`CPP12-victim-pid-clean-2run`）；不阻塞迁移推进 |
