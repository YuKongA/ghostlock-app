...
- [ ] `main.cpp` 分层与 namespace 化系列（`1ec96ab` main 解析、`172062d` stage 拆分、`d9b1042` W2/W3 拆分+enum、`300d3bf` 分 TU、`d862a51` 四单元 namespace、`fb8fb59`/`dfd0d60`/`a30e8f2` support/route/memory namespace）在 APK 283 连续两次 W1 spray 窗口 panic（`CPP12r-20260918-w1-panic`），已整体回退到 `faac9ce`（native `395c5faf…`，代码逐字节一致）；后续按“分 TU → namespace”小批次逐个门禁重试。
...
| KERNEL-PANIC-01 | Multicast 攻击存在间歇性系统崩溃：CPP06c、CPP07、CPP12b/c/e、CPP12k、CPP12p、CPP12r 分别在 spray 阶段或 W2/W1 PI-route 后触发；`CPP14-release`、`CPP12-victim-pid-clean-2run`、`CPP12p`/`CPP12q` 与 `CPP12r`（2/2 panic）/`faac9ce` 回退版复跑 PASS 均证明**同一二进制**可一次崩溃、一次通过，且崩溃集中在 **KernelSnitch collision / mm spray 及紧随的 PI-route 窗口**（另有 W1b spray 后一例）。`CPP12-victim-pid-kernelsu-crash` 的 dmesg 捕获（root）显示一类**环境相关分支**：KernelSU 已加载时攻击触发 re-enforce → system_server 崩溃（无内核 panic 打印） | 攻击层时序/状态窗口；布局敏感改动仍需多次冷机复测；门禁必须在 KernelSU 未加载的干净启动下进行 | 攻击层观察 | [ ] 已记录全部证据（`analysis/device-gates/CPP*-kernel-panic*` 含 dmesg 捕获、`pi-timeout-binary-diff.md`、`CPP12k`/`CPP12l`、`CPP12p`/`CPP12q`、`CPP12r`、`CPP14-release`、`CPP12-victim-pid-clean-2run`）；不阻塞迁移推进 |
