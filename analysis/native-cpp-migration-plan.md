...
- [x] VictimContext pid 所有权（`set_child`/`mark_child_exited`/`release_child`/`retire_child`）：历史两次尝试（`c222151`/等效重试，native `b147df9f`）在 APK 242 上 2/2 W2 spray 窗口 panic（`CPP12k-20260917-multicast-kernel-panic`），已回退（`552c8b2`），隔离复跑 PASS（`CPP12l`）。在分层/namespace 基线上重做（`victim_context.cpp` + host test），形状对比 6/8 攻击函数严格一致、`waiter_thread`/`do_one_write` 仅 1 处地址注解差异（native `e065ca70…`）；设备门禁待跑。
