...
- [x] M02 fd 段：`check_selinux_off`/`enforce_readable`/`perf_find_task` 的裸 fd → `UniqueFd`/`MappedRegion`（munmap→close 顺序不变，攻击关键函数逐指令一致）；`slab_drain` 的 pid 数组 → `std::array<ChildProcess,64>`（kill+reap 顺序不变、零堆分配）。Multicast 门禁通过 `CPP12f-20260917-multicast-pass`。
