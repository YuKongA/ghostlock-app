...
| KERNEL-PANIC-01 | Multicast 攻击存在间歇性系统崩溃：CPP06c、CPP07、CPP12b/c/e、CPP12k、CPP12p 分别在 spray 阶段或 W2/W1 PI-route 后触发；`CPP14-release`、`CPP12-victim-pid-clean-2run` 与 `CPP12p`/`CPP12q` 均证明**同一二进制**可一次崩溃、一次通过，且崩溃集中在 **KernelSnitch collision / mm spray 及紧随的 PI-route 窗口**（另有 W1b spray 后一例）。`CPP12-victim-pid-kernelsu-crash` 的 dmesg 捕获（root）显示一类**环境相关分支**：KernelSU 已加载时攻击触发 re-enforce → system_server 崩溃（无内核 panic 打印） | 攻击层时序/状态窗口；布局敏感改动仍需多次冷机复测；门禁必须在 KernelSU 未加载的干净启动下进行 | 攻击层观察 | [ ] 已记录全部证据（`analysis/device-gates/CPP*-kernel-panic*` 含 dmesg 捕获、`pi-timeout-binary-diff.md`、`CPP12k`/`CPP12l`、`CPP12p`/`CPP12q`、`CPP14-release`、`CPP12-victim-pid-clean-2run`）；不阻塞迁移推进 |
