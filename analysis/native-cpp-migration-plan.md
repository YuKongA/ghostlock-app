...
- [x] VictimContext pipe 集合收编（小步重试）：`struct child_pipes` → `ghostlock::VictimContext`（header-only，六 `UniqueFd` 顺序不变），pid 所有权暂留 `run_exploit`；二进制 640/640 形状一致、8/8 攻击关键函数严格一致、仅 `run_exploit` 两处 `bl` 注解位移（`6ab92de`）。首次冷机门禁 PASS：`CPP12m-20260917-multicast-pass`（native `625d5300…`）；按布局敏感规则待第二次冷机复跑。
- [ ] VictimContext pid 所有权（`set_child`/`retire`/`release` 与主机生命周期测试）：首次尝试 `c222151`（native `b147df9f`）在 APK 242 上 **2/2 kernel panic**（`CPP12k-20260917-multicast-kernel-panic`，均截断于 W2 spray 窗口），已回退（`552c8b2`），隔离复跑 PASS（`CPP12l`）。**待受控重试**：需在 `KERNEL-PANIC-01` 有进一步手段或接受多次冷机复测的前提下进行。
...
| KERNEL-PANIC-01 | Multicast 攻击存在间歇性内核崩溃（无 dmesg/pstore 权限）：CPP06c、CPP07、CPP12b/c/e、CPP12k 分别在 spray 阶段或 W2 后触发，位置不一致；与构建布局可能相关。CPP12k 为同构建 2/2 连续（W2 spray 窗口），回退后隔离复跑 PASS，形成“布局位移 + W2 spray 敏感”的强关联样本 | 无（需外部 panic 解析能力或多次统计）；后续布局敏感改动建议多次冷机复测 | 攻击层观察 | [ ] 已记录全部证据（`analysis/device-gates/CPP*-kernel-panic*`、`pi-timeout-binary-diff.md`、`CPP12k`/`CPP12l`）；不阻塞迁移推进 |
