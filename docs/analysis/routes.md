...
> **现状注（2026-09）**：Kotlin 现以 **stdin**（`--ghostlock-app-call`）传 GLK1 profile，route 由 profile 决定；`GHOSTLOCK_TCP_ROUTE` 等环境变量不再由 App 传递，仅在 legacy `offsets.json` 入口由 converter 读取。详见 [native-entrypoint-plan.md](native-entrypoint-plan.md)、[environment-convergence-plan.md](environment-convergence-plan.md)。下文 UML/正文里的 `--profile`、`GHOSTLOCK_TCP_ROUTE` 为历史记录。
